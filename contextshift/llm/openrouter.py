...
class OpenRouterProvider:
    """
    An LLMProvider (contextshift.llm.base.LLMProvider) backed by
    OpenRouter's REST API (https://openrouter.ai).

    OpenRouter exposes the same OpenAI-compatible chat-completions wire
    format as Groq, so this provider mirrors GroqProvider's structure:
    it owns everything OpenRouter-specific -- authentication, the exact
    request payload shape, HTTP transport, retry-on-429 behavior and
    backoff schedule, and how a streamed response is parsed into text
    chunks (manual SSE line parsing). None of this is visible through
    the LLMProvider interface -- code holding an LLMProvider has no way
    to tell it's talking to OpenRouter at all.
    ...
    messages. See docs/decisions/0006-llm-provider-interface.md for the
    reasoning.

    Args:
        api_key: OpenRouter API key. Required, and validated at
            construction time. Not read from any global configuration --
            contextshift never imports application configuration directly
            (see docs/decisions/0001-library-independence-and-adapter-placement.md).
            Whatever constructs an OpenRouterProvider (a Flask adapter, a
            CLI, a notebook) supplies it explicitly.
        model: OpenRouter model identifier, in OpenRouter's
            "vendor/model" form. Defaults to
            `meta-llama/llama-3.1-8b-instruct`.
        base_url: OpenRouter's chat-completions endpoint. Defaults to
            OpenRouter's real endpoint; overridable for testing or a
            proxy deployment.
        site_url: Optional. Sent as OpenRouter's `HTTP-Referer` header,
            which attributes traffic to your app on openrouter.ai.
            Omitted from the request entirely when None.
        app_name: Optional. Sent as OpenRouter's `X-Title` header, the
            display name for that same attribution. Omitted from the
        ...
        for attempt in range(1, _MAX_RETRIES + 1):
            try:
                ...
                if not isinstance(content, str):
                    ...
                    raise ValueError(
                        f"OpenRouter API returned a non-string content field ({content!r}) -- "
                ...
                print(f"[OPENROUTER API ERROR] attempt {attempt}: {str(e)}")
                if attempt == _MAX_RETRIES:
                    if isinstance(e, requests.exceptions.RequestException) and "429" in str(e):
                        raise Exception("OpenRouter rate limit reached. Please wait a moment and try again.")
                    raise Exception(f"Failed to call OpenRouter API: {str(e)}")
                time.sleep(_BASE_BACKOFF * attempt)

        raise Exception("OpenRouter API request failed after maximum retries.")
