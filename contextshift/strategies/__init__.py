...
policy) and SlidingWindowStrategy (a pure count-based window, no
pinning) -- that select which messages belong in an LLM's context
...
two strategies in existence, nothing in this codebase selects between
...
__all__ = ["ContextStrategy", "ContextResult", "PinnedRecencyStrategy", "SlidingWindowStrategy", "total_tokens"]
