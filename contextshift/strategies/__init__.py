...
strategy or the application that consumes them. total_tokens() is a
shared helper -- summing Message.token_count against a TokenBudget is
something essentially every budget-respecting strategy needs, so it's
exported alongside the interface rather than left as an internal detail
of the one strategy that happens to use it first.
