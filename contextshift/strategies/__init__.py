...
pinning), RecencyStrategy (pure budget-driven recency, no window, no
pinning), and SummarizationStrategy (compresses older messages into a
summary instead of discarding them, at the cost of a model call -- see
docs/decisions/0015-summarization-strategy.md) -- that select which
messages belong in an LLM's context under a token budget. New
strategies are added by implementing ContextStrategy, without
modifying any existing strategy or the application that consumes
them. total_tokens() is a shared helper -- summing
Message.token_count against a TokenBudget is something essentially
every budget-respecting strategy needs, so it's exported alongside
the interface rather than left as an internal detail of one strategy.

There is no registry for looking up strategies by name yet: even with
four strategies in existence, nothing in this codebase selects between
